...
from models.lagcl_encoder import LAGCLEncoder, Discriminator
...
class LAGCLLinkPredictionModel(torch.nn.Module):
    ...
    def __init__(
        ...
        self._node_initial = nn.Parameter(
            nn.init.xavier_uniform_(
                torch.empty(user_num + item_num, hidden_dim)))
    ...
    def split_user_item_embs(self, node_embs):
        ...
        user_embs = torch.zeros(self.user_num,
                                self.hidden_dim,
                                device=self.device)
        item_embs = torch.zeros(self.item_num,
                                self.hidden_dim,
                                device=self.device)

        user_embs[user_node_ids] = node_embs[self.is_user_node]
        item_embs[item_node_ids -
                  self.user_num] = node_embs[~self.is_user_node]
    ...
    def forward(self, data):
        ...
        task_loss = bpr_loss(user_emb=user_emb,
        ...
        L_kl_corr_user = torch.nn.functional.kl_div(
            ...
            reduction='batchmean',
            log_target=False) * self._encoder.kl_eps
        # L_kl_corr_item = torch.nn.functional.kl_div(i_emb_t[pos_idx].log_softmax(dim=-1),
        #                                             i_emb_h[pos_idx].softmax(dim=-1),
        #                                             reduction='batchmean',
        #                                             log_target=False) * self.kl_eps_item
        ...
        ], self.global_head_mask[self.is_user_node]) * self._encoder.m_h_eps
    ...
    def cal_cl_loss(self, idx, user_emb, item_emb):
        u_idx = torch.unique(torch.Tensor(idx[0]).type(torch.long)).to(
            self.device)
        i_idx = torch.unique(torch.Tensor(idx[1]).type(torch.long)).to(
            self.device)

        def exec_perturbed(x):
            random_noise = torch.rand_like(x).to(self.device)
            x = x + torch.sign(x) * F.normalize(
        ...
        item_cl_loss = InfoNCE(item_view_1[i_idx], item_view_2[i_idx],
                               self._encoder.cl_tau)
    ...
    def train_disc(self, embed_model, disc_model, optimizer_D,
        ...
        # todo: 头部点的范围是否缩小到仅 user 节点中
        # -- user + item
        # all_head_emb_h = torch.cat([u_emb_h, i_emb_h])[self.global_head_mask]
        # all_emb_t = torch.cat([u_emb_t, i_emb_t])
        # all_emb_nt = torch.cat([u_emb_nt, i_emb_nt])
        # cell_mask = self.global_head_mask
        # -- user + item
        ...
        # --- 仅 item
        # all_head_emb_h = i_emb_h[self.data.item_is_head_mask]
        # all_emb_t = i_emb_t
        # --- 仅 item
        ...
        if True:
            if self.annealing_type == 0:
                ...
                rate = 1 - min(cur_iter /
                               (annealing_stop_rate * iter_num), 1.0)
    ...
    def train_gen(self, embed_model, optimizer, disc_model, disc_pseudo_real):
        ...
        # disc 1
        all_emb_t = u_emb_t
        # all_emb_t = u_emb_t[~self.user_is_head_mask]
        # all_emb_t = i_emb_t
        # all_emb_t = torch.cat([u_emb_t, i_emb_t])
        ...
        # all_emb_nt = torch.cat([u_emb_nt, i_emb_nt])[self.global_head_mask]
...
def next_batch_pairwise(training_data, node_table, training_set_u,
                        training_set_i, batch_size):
...
def build_argparse():
    ...
    parser.add_argument("--learning_rate",
                        ...
                        default=-1,
                        help='learning_rate')
    parser.add_argument("--metric_topk",
                        type=int,
                        default=-1,
                        help='metric_topk')
    parser.add_argument("--epoch", type=int, default=-1, help='epoch')

    parser.add_argument("--noise_eps",
                        type=float,
                        default=-1,
                        help='noise_eps')
    parser.add_argument("--cl_tau", type=float, default=-1, help='InfoNCE tau')
    parser.add_argument("--cl_rate",
                        type=float,
                        default=-1,
                        help='cl rate for InfoNCE')
    parser.add_argument("--tail_k_threshold",
                        type=int,
                        default=-1,
                        help='tail_k_threshold')
    parser.add_argument("--m_h_eps", type=float, default=-1, help='')
    parser.add_argument("--kl_eps", type=float, default=-1, help='')

    parser.add_argument("--neighbor_norm_type",
                        type=str,
                        default='-1',
    ...
    for key, value in args.__dict__.items():
        if key in default_params and key not in [
                'dataset_name'
        ] and str(value) != '-1' and default_params[key] != value:
            print(
                f'default params ({key}): {default_params[key]} ---> {value}')
...
def main():
    ...
    # step 1: 构建dataset
    dataset_name = f'datasets/{params["dataset_name"]}'

    # step 4: 加载额外所需信息
    graph, node_table, train_edges, (
        training_set_u, training_set_i,
        test_set), (node_table_dict_by_node_id,
                    node_table_dict_by_node_idx) = build_graph(dataset_name)

    # 读取包含 label 的训练样本，并按照
    training_data = train_edges[['node1_id', 'node2_id']].values.tolist()
    node_raw_idx_mapping = {
        key: value['node_feature']
        for key, value in node_table_dict_by_node_id.items()
    }
    id2item = {
    ...
    optimizer_D = torch.optim.RMSprop(disc_model.parameters(),
    ...
    def train():
        ...
        train_loader = next_batch_pairwise(training_data,
                                           node_table_dict_by_node_id,
        ...
        for n, data in enumerate(pbar):
            ...
            if n % discD_every_iter == 0:
                ...
                model.train_disc(model, disc_model, optimizer_D,
                                 disc_pseudo_real, optimizer_D_pseudo_real, n,
                                 iter_num)
                if n % discG_every_iter == 0 and n > 0:
                    model.train_gen(model, optimizer, disc_model,
                                    disc_pseudo_real)
        ...
        with torch.no_grad():
            ...
            for user in tqdm(test_set):
                ...
                candidates = torch.matmul(
                    user_embedding,
                    item_embeddings.t()).detach().cpu().numpy()
                for item in training_set_u[user].keys():
                    candidates[node_raw_idx_mapping[item] -
                               model.user_num] = -10e8
    ...
    for epoch in range(1, params['epoch'] + 1):
        ...
        best_metrics['Recall@20'] = max(best_metrics.get('Recall@20', 0.0),
                                        recall)
        best_metrics['NDCG@20'] = max(best_metrics.get('NDCG@20', 0.0), ndcg)
        t2 = time.time()
        print(
            "Epoch: {:02d}, Loss: {:.4f}, Recall@20: {:.4f}, NDCG@20: {:.4f}, best Recall@20: {:.4f}, best NDCG@20: {:.4f}, train_time: {:4f}, val_time: {:4f}"
            .format(epoch, loss, recall, ndcg, best_metrics['Recall@20'],
                    best_metrics['NDCG@20'], t1 - t0, t2 - t1))
